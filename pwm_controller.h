	...
	if (pmref.SvmEnable == false && pmref.ThiEnable == false) {
		...
		voltage_coefficient =
		    pmref.mVoltage < 0.786f
		        ? 4.0f / M_PI
		        : 4.0f / M_PI + pow(2.267303569733523f * CORRECTIONFACTOR_NORMAL[(byte)min(213, (pmref.mVoltage - 0.786f) * 1000)] / 255, 3);
	} else if (pmref.SvmEnable == false && pmref.ThiEnable == true) {
		voltage_coefficient =
		    pmref.mVoltage < 0.906f
		        ? 4.0f / M_PI
		        : 4.0f / M_PI + pow(1.9451517598583832f * CORRECTIONFACTOR_THI[(byte)min(92, (pmref.mVoltage - 0.907f) * 1000)] / 255, 3);
	} else if (pmref.SvmEnable == true && pmref.ThiEnable == false) {
		voltage_coefficient =
		    pmref.mVoltage < 0.906f
		        ? 4.0f / M_PI
		        : 4.0f / M_PI + pow(1.9439334827663977f * CORRECTIONFACTOR_SVM[(byte)min(92, (pmref.mVoltage - 0.907f) * 1000)] / 255, 3);
	}
...
void UpdatePwmMode(PulseModeReference pmref, pwm_config* pm) {
	...
	if (pm->carrier_freq_hz > 0) {
		increment = (uint32_t)((pm->signal_freq_hz / (2.0f * pm->carrier_freq_hz)) * SIN_LENGTH * SHIFT);
	}
